        ...
        for layer in ${layer_size_list[@]}; do                   
            for lr in ${lr_list[@]} ; do
                for dropout in  ${dropout_list[@]}; do
                    ...
                    job_name="$2-29-${layer}x${num_layers}-${dropout}"
                    ...
                    qsub $1 -N $job_name  $pbs_account  $pbs_allocate   $pbs_folders \
                        -v epochs=$epochs,batch_size=$batch_size,num_layers=$num_layers,layer=$layer,dropout=$dropout,datadir=$datadir,outdir=$outdir,lr=$lr,dev=$dev,config=$config
                done
            done
        done
    done
